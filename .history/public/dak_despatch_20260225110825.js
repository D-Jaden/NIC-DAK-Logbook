...
async function saveToDatabase() {
    ...
    try {
        ...
        if (result.success) {
            ...
            changedRows.forEach(rowIndex => {
                if (tableData[rowIndex]) {
                    originalData.set(tableData[rowIndex].id, createRowHash(tableData[rowIndex]));
...
async function getClient() {
    if (!gradioClient) {
        const { Client } = await import("https://esm.sh/@gradio/client");
        gradioClient = await Client.connect("YOUR_HF_USERNAME/YOUR_SPACE_NAME");
    }
    return gradioClient;
}
...
async function translateText(text) {
    ...
    try {
        const client = await getClient();
        const result = await client.predict("/translate", { data: [text] });
        const translated = result.data[0];
        translationCache.set(text, translated);
        return translated;
    } catch (error) {
...
async function translateTextBatch(texts) {
    const client = await getClient(); // connect once, reuse
    const results = await Promise.all(
        texts.map(async (text) => {
            if (translationCache.has(text)) return translationCache.get(text);
            try {
                const result = await client.predict("/predict", { data: [text] });
                const translated = result.data[0];
                translationCache.set(text, translated);
                return translated;
            } catch {
