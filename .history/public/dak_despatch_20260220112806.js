...
function syncTableDataWithDOM() {
    ...
    rows.forEach((row, index) => {
        if (!tableData[index]) {
            console.warn(`¸ No tableData for row ${index}`);
            return;
        }
...
async function translateText(text) {
    ...
    try {
        ...
        const submitRes = await fetch(
            "https://d-jaden02-krutrim-english-hi-translation.hf.space/call/predict",
            {
                method: 'POST',
        ...
        const resultRes = await fetch(
            `https://d-jaden02-krutrim-english-hi-translation.hf.space/call/predict/${event_id}`
        );

        const raw = await resultRes.text();
        // Gradio streams SSE lines, grab the data line
        const match = raw.match(/^data:\s*(.+)$/m);
        if (!match) throw new Error('No data in response');
...
// Batch: just run translateText in parallel
async function translateTextBatch(texts) {
