...
function initializeTable() {
    ...
    if (window.tableInitialized) {
        console.log('â­ï¸ Table already initialized, skipping...');
        return;
    }
    ...
    if (preservedData && preservedRowCount) {
        console.log('ðŸ”„ Restoring data from previous page...');
        ...
        console.log('âœ… Data restored from page switch!');
        return; 
    }
    ...
    if (userIsAuthenticated) {
        console.log('ðŸ”„ Authenticated user - loading data...');
        loadUserData(); // This will handle BOTH cases: existing data OR new user
    } else {
        console.log('ðŸ“ Guest user - initializing with 6 empty rows...');
        for (let i = 0; i < 6; i++) {
            addNewRow();
        }
        rebuildTable();
    ...
    if (loadBtn) {
        ...
        console.error('âŒ Load button not found!');
    }*/
    ...
    if (undoBtn) {
        ...
        console.log('âœ… Undo button listener attached');
    }
    ...
    if (redoBtn) {
        ...
        console.log('âœ… Redo button listener attached');
    }
...
function syncTableDataWithDOM() {
    ...
    rows.forEach((row, index) => {
        if (!tableData[index]) {
            console.warn(`âš ï¸ No tableData for row ${index}`);
            return;
        }
        ...
        // Handle radio buttons (not caught by .cell selector)
        ...
        radioCells.forEach(radioCell => {
            ...
            if (field && checkedRadio) {
                tableData[index][field] = checkedRadio.value;
                console.log(`  âœ… Saved radio ${field}: "${checkedRadio.value}"`);
            }
        });
    });
}
...
async function loadUserData() {
    if (window.isLoadingData) {
        console.log('â­ï¸ Already loading data, skipping duplicate call...');
        return;
    }
    ...
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            console.log(`ðŸ“Š Loaded ${result.data.length} existing records`);
            ...
            console.log('âœ… User data loaded and displayed');
...
async function saveToDatabase() {
    ...
    const confirmMessage = `Save ${totalChanges} changes?\n\n` +
        `â€¢ ${newRowsData.length} new rows\n` +
        `â€¢ ${changedRowsData.length} modified rows`;
        
    if (!confirm(confirmMessage)) {
        return;
    }

    console.log(`ðŸ”„ Saving ${totalChanges} changed rows...`);
    
    try {
        const saveBtn = document.querySelector('.save-btn');
        const originalText = saveBtn.textContent;
        saveBtn.textContent = 'â³ Saving Changes...';
        ...
        if (result.success) {
            ...
            saveBtn.textContent = 'âœ… Changes Saved!';
            setTimeout(() => {
        ...
        alert('âŒ Error saving changes: ' + error.message);
    } finally {
        const saveBtn = document.querySelector('.save-btn');
...
async function translateText(text) {
    console.log('ðŸ”„ Translation requested for:', text);
...
async function saveData(cell) {
    ...
    if (tableData[row]) {
        ...
        if (translatableColumns.includes(field) && !field.endsWith('Hindi') && value) {
            ...
            console.log('ðŸ” Looking for Hindi field:', hindiField);
            console.log('ðŸ” Found element:', hindiInput);
            
            if (hindiInput) {
                // Strip HTML tags for translation
                const textToTranslate = value.replace(/<[^>]*>/g, '');
                console.log('ðŸ”„ Translating:', textToTranslate);
                ...
                console.warn('âš ï¸ Hindi textarea not found for field:', hindiField);
            }
        }
